...
from .chatbot import chat_incident_prompt, chat_user_prompt, load_chat_history_from_db
...
class ChatApp:
    ...
    def render_login_page(self):
        """Render login page"""
        st.title("🔐 Log in to VSAT Chatbot")
    ...
    def render_main_app(self):
        ...
        if user_vectordb_key in st.session_state:
            ...
            if incident_request_key in st.session_state:
                incident_id = st.session_state.pop(incident_request_key)
                incident = get_incident_by_id(incident_id)
                if incident:
                    st.session_state[chat_history_key] = chat_incident_prompt(
                        incident=incident,
                        chat_history=st.session_state[chat_history_key],
                        vectordb=st.session_state[user_vectordb_key],
                        username=username,
                    )
                else:
                    st.warning("Incident not found for AI assistant.")
